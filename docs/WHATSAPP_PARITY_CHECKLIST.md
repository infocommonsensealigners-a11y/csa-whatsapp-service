...
| WhatsApp Web | Estado | Baileys | Prio |
|---|---|---|---|
| Sincronización inicial del historial sin duplicar y fusionada. | **OK** — `messaging-history.set` al canónico, PK idempotente; `pnJid/lidJid` aprendidos antes de escribir. ⚠️ Pendiente de comprobar en el re-escaneo real (nunca ha entrado un history sync en esta base). | `messaging-history.set` | P0 |
...
| Reconexión sin huecos ni duplicados; indicador visible. | **OK** — WhatsApp re-entrega lo perdido como `append`; PK evita duplicados; pie del teléfono y píldora dicen el estado real. **Parcial**: una desconexión larga solo se cierra con re-emparejamiento (WhatsApp no reenvía la lista con credenciales ya emparejadas). | `connection.update`, `append` offline | P0 |
