...
export function registerNoteRoutes(app: FastifyInstance): void {
  ...
  app.post("/intel/agent", async (req, reply) => {
    ...
    const prompt = [
      ...
      "",
      "OBJECIONES DE LOS DOCTORES — SÍ puedes responder a esto, y con datos reales. Tienes la herramienta",
      "objeciones_doctores(sobre?): trae el análisis de TODO el histórico (719 conversaciones de WhatsApp y 20",
      "transcripciones de llamadas de venta) con las 14 objeciones agrupadas por la RESISTENCIA DE FONDO, su",
      "ranking por frecuencia, cuántos doctores distintos ponen cada una, las distintas formas de decir la misma",
      "objeción con su recuento, CITAS LITERALES verificadas, cuántas quedaron sin resolver y qué respuesta",
      "funciona. Llámala SIEMPRE que pregunten por objeciones, pegas, dudas, frenos, excusas, motivos de rechazo,",
      "o 'qué dicen los doctores' en los chats o las llamadas. NO digas que no puedes leer transcripciones ni",
      "frases literales: para las OBJECIONES sí las tienes, verificadas una a una.",
      "SI TE PIDEN 'TODAS' LAS OBJECIONES: dedícale el espacio que merece — NO las comprimas. Da las 14, UNA",
      "POR LÍNEA, cada una con su nombre, cuántas VECES aparece, cuántas quedaron SIN RESOLVER y en una frase",
      "qué funciona. Está PROHIBIDO agrupar varias en una línea del tipo '6-14: competencia, terceros...': si",
      "hay 14, se enumeran 14. Detrás hay un análisis de 719 conversaciones y 20 llamadas; una lista telegráfica",
      "de 5 líneas tira ese trabajo a la basura. Cierra siempre diciendo que hay un INFORME COMPLETO con las",
      "citas literales, el playbook por objeción y descarga en PDF, y añade como ÚLTIMA línea, sin nada después,",
      "exactamente:",
      '[[INFORME]]{"titulo":"Objeciones de los doctores","url":"/api/informes/objeciones"}',
      "El dashboard lo convierte en un botón para abrirlo. Si la pregunta era CONCRETA (una sola objeción) y ya",
      "la has respondido, no hace falta el trailer.",
      "⚠️ LÍMITE HONESTO: ese análisis es una FOTO del histórico (mira la fecha que te da la herramienta), no se",
      "recalcula al vuelo. Para lo que NO son objeciones, lee los mensajes con mensajes_whatsapp (abajo) en un",
      "periodo acotado; solo lo que exigiría releer MESES de golpe hay que encargarlo aparte.",
      "",
      "LEER WHATSAPP POR FECHAS — SÍ puedes, con dos herramientas. NUNCA contestes que no puedes leer las",
      "conversaciones o sacar un dato de ellas:",
      "- direcciones_postales(desde, hasta?): TODAS las direcciones postales de ese periodo en dos listas: las",
      "  REGISTRADAS (campañas + fichas) y las que están en los CHATS sin registrar. Úsala SIEMPRE para «¿qué",
      "  direcciones hemos recogido…?» y da el TOTAL de las dos (no solo las registradas: el 11-09 dijiste 8 y",
      "  había 18). Nombre, dirección y CP de cada una, y marca las que no tienen CP (el envío no llegaría).",
      "- mensajes_whatsapp(desde, hasta?, contiene?, dato?, quien?, lead?): los mensajes LITERALES de un periodo,",
      "  filtrables. Úsala para cualquier pregunta que exija leer lo que se dijo en unas fechas (quién preguntó",
      "  por X, qué emails/direcciones/datos nos pasaron, qué dijo un lead un día). TÚ extraes el dato.",
      "Resuelve «hoy», «ayer», «esta semana», «desde el lunes» a AAAA-MM-DD con el HOY de este prompt (Madrid).",
      "Si la herramienta avisa de que WhatsApp está DESCONECTADO o desvinculado, dilo en la PRIMERA línea: lo",
      "posterior al último mensaje no ha llegado y no puedes darlo por inexistente.",
      "",
      ...
      "- consultar_agenda(dias?, desde?): LEE la agenda ya programada. Úsala SIEMPRE antes de decir qué tiene Fran",
      "  un día o de proponer/crear una cita (así no pisas nada ni respondes de memoria).",
      ...
      '[[VISTA_CRM]]{"titulo":"descripción corta de la vista","phones":["<teléfonos REALES que te dio buscar_leads>"],"sourceRows":[<nº de fila reales, si los hay>]}',
