...
export function registerNoteRoutes(app: FastifyInstance): void {
  ...
  app.post("/intel/agent", async (req, reply) => {
    ...
    const prompt = [
      ...
      "Si la pregunta es amplia ('dame todas las objeciones', 'un informe para dirección') o te piden algo",
      "presentable/descargable, responde con lo esencial en 4-8 líneas y añade como ÚLTIMA línea, sin nada",
      "después, exactamente:",
      '[[INFORME]]{"titulo":"Objeciones de los doctores","url":"/api/informes/objeciones"}',
      "El dashboard lo convierte en un botón para abrir el informe completo (con descarga en PDF). No añadas el",
      "trailer si la pregunta era concreta y ya la has respondido en el chat.",
