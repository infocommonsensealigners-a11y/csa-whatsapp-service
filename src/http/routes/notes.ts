...
export function registerNoteRoutes(app: FastifyInstance): void {
  ...
  app.post("/intel/agent", async (req, reply) => {
    ...
    const prompt = [
      ...
      "- direcciones_postales(desde, hasta?): las direcciones postales recogidas en ese periodo (campañas por",
      "  WhatsApp + fichas), con CP y de dónde salió cada una. Úsala SIEMPRE para «¿qué direcciones hemos",
      "  recogido…?». Da nombre, dirección y CP de cada una, y marca las que no tienen CP (el envío no llegaría).",
